...
import type { ExecutionContext, StepResult, ToolHandler } from "../types.js";
...
function resolveParamValue(value: unknown, results: Map<string, StepResult>): unknown {
  if (typeof value !== "string") return value;
  ...
  if (exact) {
    ...
    if (!exact[2]) return base;
    return exact[2].split(".").reduce<unknown>((acc, key) => {
...
function resolveParams(params: JsonMap | undefined, results: Map<string, StepResult>): JsonMap {
  if (!params) return {};
...
async function readNotes(filePath: string): Promise<Record<string, string[]>> {
  try {
...
async function writeNotes(filePath: string, notes: Record<string, string[]>): Promise<void> {
  await ensureDir(filePath);
  await fs.writeFile(filePath, `${JSON.stringify(notes, null, 2)}\n`, "utf8");
}

function normalizePath(cwd: string, projectRoot: string, input: string): string {
  if (path.isAbsolute(input)) return input;
  const local = path.resolve(cwd, input);
  if (path.parse(input).root) return input;
  if (input.startsWith(".") || input.includes(path.sep)) return local;
...
function quarterMatches(date: Date, query: string): boolean {
  const month = date.getMonth();
  const year = date.getFullYear();
  const q = Math.floor(month / 3) + 1;
  const target = query.toLowerCase();
  if (target.includes("q1")) return q === 1;
  if (target.includes("q2")) return q === 2;
  if (target.includes("q3")) return q === 3;
  if (target.includes("q4")) return q === 4;
  if (/\b2025\b/.test(target)) return year === 2025;
  if (/\b2026\b/.test(target)) return year === 2026;
  return true;
}

function timeMatches(stat: Awaited<ReturnType<typeof fs.stat>>, query?: unknown): boolean {
  if (typeof query !== "string" || !query.trim()) return true;
  const target = query.toLowerCase();
  const mtime = stat.mtime;
  const now = new Date();
  if (target.includes("today")) return mtime.toDateString() === now.toDateString();
  ...
  if (target.includes("this year")) return mtime.getFullYear() === now.getFullYear();
...
const handlers: Record<string, ToolHandler> = {
  async file_search(params, ctx) {
    ...
    const candidates: Array<{ path: string; score: number; mtime: number }> = [];

    for (const filePath of paths) {
      const stat = await fs.stat(filePath);
      if (!timeMatches(stat, params.time)) continue;
      const base = path.parse(filePath).name.toLowerCase();
      const suffix = path.extname(filePath).replace(/^\./, "").toLowerCase();
      if (ext && suffix !== ext) continue;
      if (filename && !base.includes(filename) && !path.basename(filePath).toLowerCase().includes(filename)) continue;

      let score = 0;
      if (filename && base === filename) score += 100;
      if (filename && base.includes(filename)) score += 40;
      if (ext) score += 20;
      if (folder) score += 5;
...
async function runShell(cmd: string, args: string[], cwd = process.cwd()): Promise<{ stdout: string; stderr: string; code: number }> {
...
export function resolveToolParams(step: { params?: JsonMap }, results: Map<string, StepResult>): JsonMap {
